...
class PairOverlookerWithSurveillanceUseCase(
    ...
    suspend fun execute(surveillanceUUID: String, overlookerUUID: String): Result<Unit> {
        Log.d(_tag, "Attempting to pair Overlooker: $overlookerUUID with Surveillance: $surveillanceUUID")
        return try {
            ...
            Log.d(_tag, "Adding Overlooker ($overlookerUUID) to Surveillance ($surveillanceUUID) in Firebase.")
            firebaseService.addOverlookerToSurveillance(surveillanceUUID, overlookerUUID)
            Log.d(_tag, "Overlooker ($overlookerUUID) added to Surveillance ($surveillanceUUID) in Firebase.")
            ...
            if (!notificationRepository.checkIfFCMTokenAreSavedInDB(surveillanceUUID)) {
                Log.w(_tag, "FCM token not found for Surveillance Device ($surveillanceUUID). Attempting to save using Overlooker UUID ($overlookerUUID).")
                mainRepository.saveFCMTokenToFirebase(overlookerUUID)
                Log.d(_tag, "Attempted to save FCM token for Surveillance Device ($surveillanceUUID) using Overlooker UUID ($overlookerUUID).")
            } else {
            ...
            if (!notificationRepository.checkIfFCMTokenAreSavedInDB(overlookerUUID)) {
                Log.w(_tag, "FCM token not found for Overlooker Device ($overlookerUUID). Attempting to save using Surveillance UUID ($surveillanceUUID).")
                mainRepository.saveFCMTokenToFirebase(surveillanceUUID)
                Log.d(_tag, "Attempted to save FCM token for Overlooker Device ($overlookerUUID) using Surveillance UUID ($surveillanceUUID).")
            } else {
                Log.d(_tag, "FCM token found for Overlooker Device ($overlookerUUID).")
            }

            Log.d(_tag, "Pairing successful between Overlooker: $overlookerUUID and Surveillance: $surveillanceUUID.")
            Result.success(Unit)
        } catch (e: Exception) {
            Log.e(_tag, "Error during pairing of Overlooker: $overlookerUUID with Surveillance: $surveillanceUUID", e)
            Result.failure(e)
        }
    }
}